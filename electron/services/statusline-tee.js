...
function shouldUpdateLive(nextText) {
  ...
  if (!nextPath || !prevPath || nextPath === prevPath) return true;

  const nextMtime = fileMtimeMs(nextPath);
  const prevMtime = fileMtimeMs(prevPath);
  if (!nextMtime || !prevMtime) return true;
...
process.stdin.on("end", () => {
  try {
    if (data.trim() && shouldUpdateLive(data)) fs.writeFileSync(livePath, data, "utf8");
  } catch {}
